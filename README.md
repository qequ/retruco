...
1. Python 3.8+
2. Tkinter (only for the GUI; it ships with most Python installs)

# Installation

From a release artifact:
```
pip install retruco-0.1.0-py3-none-any.whl
```
...
Show the generated opcodes and per-instruction stack states:
```
retruco --debug path/to/program.timba
```

Without installing, from the source tree:
```
python -m retruco path/to/program.timba
```
...
MIENTRAS LA PILA INPUT NO ESTA VACIA
    ...
    MIENTRAS LA PILA TMPSTACK NO ESTA VACIA
        SI LA CARTA ES DE MENOR VALOR QUE TOPE DE PILA TMPSTACK
